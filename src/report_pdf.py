...
def generate_pdf(json_data, filename="report.pdf"):
    ...
    # PAGE 4: DIVIDENDS SUMMARY (Totals)
    ...
    if data['monthly_dividends']:
        ...
        for month_key, group in itertools.groupby(sorted_divs, key=get_month_key):
            # Header: e.g. "January 2024"
            y, m = month_key.split('-')
            m_name = month_names.get(m, m)
            elements.append(Paragraph(f"{m_name} {y}", h3_style))
            ...
            elements.append(Spacer(1, 15)) # Space between months
