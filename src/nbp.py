...
def _load_year_cache(currency, year):
    ...
    if cache_key in _MEMORY_CACHE: return _MEMORY_CACHE[cache_key]
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            with open(file_path, 'w') as f: json.dump(rates_map, f)
    except: pass
    _MEMORY_CACHE[cache_key] = rates_map
    return rates_map
def get_nbp_rate(currency: str, date_str: str, attempt=0) -> Decimal:
    if currency.upper() == "PLN": return Decimal("1.0")
    if attempt > 10: return Decimal("0.0")
    year = date_str[:4]
    rates_map = _load_year_cache(currency, year)
    if date_str in rates_map: return Decimal(str(rates_map[date_str]))
