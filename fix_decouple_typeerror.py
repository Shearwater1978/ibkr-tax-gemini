# fix_decouple_typeerror.py
import os

def fix_decouple_typeerror(filepath="tests/test_db_connector.py"):
    """
    Fixes TypeError in tests by adjusting mock side_effect to accept *args and **kwargs.
    """
    print(f"🛠️ Applying TypeError fix to {filepath}...")
...
def mock_decouple_config():
    ...
    with patch('src.db_connector.config') as mock_config:
        # FIX 1: Use *args, **kwargs to safely capture 'default=' passed by DBConnector
        def side_effect(key, *args, **kwargs):
            if key == 'DATABASE_PATH':
                return DB_PATH
            if key == 'SQLCIPHER_KEY':
                return DB_KEY
            return kwargs.get('default', '') # Handle the actual default if needed
...
def test_initialization_missing_key(mock_decouple_config):
    ...
    with patch('src.db_connector.config') as mock_config_missing:
        # FIX 2: Use *args, **kwargs in the side_effect for the same reason
...
"""
    ...
    try:
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(new_content)
        print("✅ tests/test_db_connector.py successfully fixed the TypeError.")
    except Exception as e:
        print(f"❌ Error writing file: {e}")

if __name__ == "__main__":
    fix_decouple_typeerror()